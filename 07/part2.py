...
def eval(cards):
    ...
    counts = sorted([(c, cards.count(c)) for c in c_set], key=lambda x: x[-1] if x[0] != 'J' else -1, reverse=True)
    counts = sorted([(c, cards.replace('J', counts[0][0]).count(c)) for c in c_set], key=lambda x: x[-1] if x[0] != 'J' else -1, reverse=True)
